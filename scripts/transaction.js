...
export async function loadTransactions(filter = "all") {
  ...
  try {
    // ------------------- FIX #1 -------------------
    // Point to the "sales" collection, not "transactions"
    ...
    snapshot.forEach(docSnap => {
      ...
      // Updated: The new 'items' array has 'name' and 'quantitySold'
      const items = data.items?.map(i => `${i.quantitySold}× ${i.name}`).join("<br>") || "—";
      ...
      const paymentMethod = data.paymentMethod || "N/A"; // You can add this field to your sales doc later
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (transactionsTab) {
    // Also load transactions on the first page load, not just on click
    loadTransactions(); 
    transactionsTab.addEventListener("click", () => loadTransactions());
  }
